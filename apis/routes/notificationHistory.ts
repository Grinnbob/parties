...
const Base = 'notificationHistory';
interface NOTIFICATION_HISTORY {
  id: string;
}

export const create = async (data: NOTIFICATION_HISTORY) => {
  const response = await API.postApi(`${Base}/`, data);
  return response;
};

export const update = async (data: NOTIFICATION_HISTORY) => {
