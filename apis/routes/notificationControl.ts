...
const Base = 'notificationControl';
interface NOTIFICATION_CONTROL {
  id: string;
  notificationType1?: boolean;
}

export const create = async (data: NOTIFICATION_CONTROL) => {
  const response = await API.postApi(`${Base}/`, data);
  return response;
};

export const update = async (data: NOTIFICATION_CONTROL) => {
